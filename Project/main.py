...
while gen_sw:
    ...
    if park_sw and not (config_sw or verify_sw):
        sc.close()
            ...
            if event == "-ADDCAR-":
                if chars.isalpha() and len(chars) == 3 and nums.isnumeric() and len(nums) == 3:
                    ...
                    if sw == 0:
                        ...
                        if Park.size == 0:
                            sc["-PLATEERROR-"].update(
                                "Su tamaño actual es 0, por favor configúrelo")
                        else:
                            sc["-PLATEERROR-"].update(
                                "No hay puestos disponibles")
                    elif sw == -2:
                        sc["-PLATEERROR-"].update(
                            "Su precio actual es 0, por favor configúrelo")
                else:
            ...
            if event == "-CHANGESIZE-":
                if values["-NEWSIZE-"].isnumeric():
                    if int(values["-NEWSIZE-"]) < len(Park.cars) or int(values["-NEWSIZE-"]) == 0:
                        sc["-SIZEERROR-"].update(
                            "ERROR: El nuevo tamaño es menor a la cantidad de carros actual o 0")
                    else:
                ...
                if values["-NEWPRICE-"].isnumeric():
                    if int(values["-NEWPRICE-"]) <= 0:
                        sc["-PRICEERROR-"].update(
                            "ERROR: El nuevo precio es ilógico")
                    else:
